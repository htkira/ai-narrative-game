...
5. refutableByClueIds引用下方线索列表中的clueId，该线索的内容必须能**直接否定**此观点的wrongPremise。注意：线索列表中既有包含决定性专业判断的"关键线索"，也有仅描述表面现象的"补充线索"和来自false_clue的"干扰线索"。refutableByClueIds**只能填入关键线索**——即摘要中包含明确的专业结论、能与wrongPremise形成直接矛盾的那些线索。仅仅描述现象（如"有骨粉散落""有打斗痕迹"）而没有做出推翻性判断的线索不能填入
...
- 为后续逐条指控做铺垫
- **格式要求（极重要）**：开场白必须是NPC说的一段纯对话文字，**禁止使用括号描写动作或场景**（如"（他猛地推开门）""（拍桌子）"等）。NPC的情绪和动作应通过对话内容本身来传达，而不是用括号旁白。正确示例："呔！你这游方郎中，怎的还在此处磨蹭？……"；错误示例："（急促的脚步声由远及近）郎中！郎中可在里面？！（棚屋门被猛地推开）……"`),
